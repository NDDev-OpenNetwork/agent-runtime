...
[private security advisory form](https://github.com/NDDev-OpenNetwork/agent-runtime/security/advisories/new).
...
A release is an annotated `vMAJOR.MINOR.PATCH` tag. The Go module proxy serves
the library from that tag directly, so `go get` is the verifiable path for the
packages: the proxy records an immutable hash for the version, and `go.sum`
fails the build if it ever changes.

The GitHub Release additionally carries CLI binaries and a `SHA256SUMS` file.
Verify a downloaded archive against that file before running it.
