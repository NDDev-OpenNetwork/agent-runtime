...
- The module moved to `github.com/NDDev-OpenNetwork/agent-runtime` and is
  published as open source under AGPL-3.0. Import paths change accordingly;
  the packages, their contracts and their behaviour do not.

### Removed

- The repository-governance, provenance, signature, module-tidy, goal-journal,
  CI-contract and release-contract checkers, and the JSON contracts they read.
  Each verified that a document describing this repository matched the
  repository, which bound the code to one GitHub account and one owner — a fork
  could not pass its own CI. Releases are now a tag plus built binaries, which
  is what the Go module proxy already guarantees.
...
[Unreleased]: https://github.com/NDDev-OpenNetwork/agent-runtime/compare/v0.1.3...HEAD
[0.1.3]: https://github.com/NDDev-OpenNetwork/agent-runtime/releases/tag/v0.1.3
[0.1.2]: https://github.com/NDDev-OpenNetwork/agent-runtime/releases/tag/v0.1.2
[0.1.1]: https://github.com/NDDev-OpenNetwork/agent-runtime/releases/tag/v0.1.1 (tag only; no release)
[0.1.0]: https://github.com/NDDev-OpenNetwork/agent-runtime/releases/tag/v0.1.0 (tag only; no release)
